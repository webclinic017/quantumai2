...
def optimize_optuna_FlorianPortfolioEnvMultiReward(env_train, env_validate, callbacks, n_trial_runs=100):
    ...
    def objective(trial):
        ...
        env_train.reward_scaling = reward_scaling
        #hmax optimization
        hmax = trial.suggest_int("hmax", 1000, 50000)
        env_train.hmax = hmax
        #sharpe_weight scaling
        sharpe_ratio_weight = trial.suggest_float("sharpe_ratio_weight", 0.1, 0.9, log=True)
        env_train.sharpe_ratio_weight = sharpe_ratio_weight
        #loss pentalty weight
        loss_penalty_weight = trial.suggest_float("loss_penalty_weight", 0.1, 0.9, log=True)
